...
buttonStart.addActionListener((ae)->{mr.line=mr.metroRuch();}); 
panelWest.add(buttonStop); //buttonStop.setFont(kursiv);
buttonStop.addActionListener((ae)->{mr.metroStop(mr.getline());}); 
for(int i=0; i<22; i++){
	buttonWest[i]=new JButton("W "+i);//buttonWest[i].setFont(kursiv);
